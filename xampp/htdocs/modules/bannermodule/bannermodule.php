...
class BANNERMODULE extends Module
{
	...
	{
		$this->name = 'bannermodule';
		$this->description = 'Creates snow animation';
		$this->tab = 'front_office_features';
		$this->version = '1.1.3';
		$this->author = 'emotionLoop';
		$this->need_instance = 0;
		$this->ps_versions_compliancy = array('min' => '1.5', 'max' => '1.7');
		$this->monolanguage_content = 'Bannermodule_MONOLANGUAGE';
		$this->text_editor_content = 'Bannermodule_TEXTEDITOR';
		$this->content_wrapper = 'Bannermodule_CONTENTWRAPPER';
		$this->content_wrapper_class = 'Bannermodule_CONTENTWRAPPER_CLASS';
		$this->content_wrapper_id = 'Bannermodule_CONTENTWRAPPER_ID';
		$this->bootstrap = true;
		$this->_html = '';
		...
		$this->displayName = $this->l('BannerModule');
		$this->description = $this->l('Creates snow animation');
	...
	public function install()
	{
		...
		if (!parent::install() || !$this->registerHook('header') || !$this->registerHook('footer') || !BANNERMODULEModel::createTables())
			return false;
	...
	public function addFilesToTemplate($path = null)
	{
		...
		$files_data = BANNERMODULEModel::getFilesInUse( $this->selected_store_id, $this->selected_language_id );
	...
	{
		$content_query = BANNERMODULEModel::getContent( $this->selected_store_id, $this->selected_language_id );
		$pre_content = '';
		$pos_content = '';
		...
		if (Tools::isSubmit('submit'.$this->name))
		{
				...
				&& (int)Configuration::get('PS_LANG_DEFAULT') == $this->selected_language_id ))
			{
				...
				if (Tools::getValue('content_text') !== false && $this->ignore_changes_content_changes == false)
					BANNERMODULEModel::setContent( Tools::getValue('content_text'), $this->selected_store_id, $this->selected_language_id );

				//store the files to be used
				if (Tools::getValue('headerFiles') !== false)
					BANNERMODULEModel::setFiles(
	...
	public function displayForm()
	{
		...
		$helper->fields_value['headerFiles[]'] = $this->processSelectFilesForMultiselect(
																BANNERMODULEModel::getFilesInUse( $this->selected_store_id, $this->selected_language_id )
																);
...
class BANNERMODULEModel extends ObjectModel
{
	...
	public static function createTables()
	{
		...
		return ( BANNERMODULEModel::createFilesTable()
				&& BANNERMODULEModel::createContentTable());
	}
