...
class SOCIALMEDIA extends Module
{
	public function __construct()
	{
		...
		$this->monolanguage_content = 'Socialmedia_MONOLANGUAGE';
		$this->text_editor_content = 'Socialmedia_TEXTEDITOR';
		$this->content_wrapper = 'Socialmedia_CONTENTWRAPPER';
		$this->content_wrapper_class = 'Socialmedia_CONTENTWRAPPER_CLASS';
		$this->content_wrapper_id = 'Socialmedia_CONTENTWRAPPER_ID';
		$this->bootstrap = true;
		$this->_html = '';
		...
		$this->displayName = $this->l('Socialmedia');
		$this->description = $this->l('Social media');
	...
	public function install()
	{
		...
		if (!parent::install() || !$this->registerHook('header') || !$this->registerHook('footer') || !SOCIALMEDIAModel::createTables())
			return false;
	...
	public function addFilesToTemplate($path = null)
	{
		...
		$files_data = SOCIALMEDIAModel::getFilesInUse( $this->selected_store_id, $this->selected_language_id );
	...
	{
		$content_query = SOCIALMEDIAModel::getContent( $this->selected_store_id, $this->selected_language_id );
		$pre_content = '';
		$pos_content = '';
		...
		if (Tools::isSubmit('submit'.$this->name))
		{
				...
				&& (int)Configuration::get('PS_LANG_DEFAULT') == $this->selected_language_id ))
			{
				...
				if (Tools::getValue('content_text') !== false && $this->ignore_changes_content_changes == false)
					SOCIALMEDIAModel::setContent( Tools::getValue('content_text'), $this->selected_store_id, $this->selected_language_id );

				//store the files to be used
				if (Tools::getValue('headerFiles') !== false)
					SOCIALMEDIAModel::setFiles(
	...
	public function displayForm()
	{
		...
		$helper->fields_value['headerFiles[]'] = $this->processSelectFilesForMultiselect(
																SOCIALMEDIAModel::getFilesInUse( $this->selected_store_id, $this->selected_language_id )
																);
...
class SOCIALMEDIAModel extends ObjectModel
{
	...
	public static function createTables()
	{
		...
		return ( SOCIALMEDIAModel::createFilesTable()
				&& SOCIALMEDIAModel::createContentTable());
	}
